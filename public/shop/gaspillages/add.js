var KTpdv = function () {
    ...
    function ProductAuPanier(donnees){
        ...
        $('#tabpanier-id').append(
            ...
            '</td>'+
            '<td class="w-600px">'+
                '<select name="motif_gaspillage[]" style="background-color: #e9ecef"  class="form-select form-select-solid h-30px p-1 rounded-0 motif_gaspillage"  required id="motif_gaspillage_id'+p+'"></select>' +
            '</td>'+
            '<td class="w-300px">'+
                '<input type="number" id="qte-'+p+'" required name="quantite[]" step="0.01" min="0"  class="form-control easy-put rounded-0 h-30px p-1 quantity_product" value="1"/>'+
            '</td>'+
        ...
        var motif_gaspillage_id = "motif_gaspillage_id"+p;
        unit_group_select_product(unit_id, donnees.id);
        selectionnerMotifGaspillage(motif_gaspillage_id);
    };
...
$("#btn-save-gaspillage").on("click",function(event){
    event.preventDefault();
    var submitButton;
    var form = document.getElementById('addForm');
    var countRow = document.getElementById('product_count').value;
    var action = form.getAttribute('action');
    submitButton = document.querySelector('#btn-save-gaspillage');
