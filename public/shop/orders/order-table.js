var KTDatatablesAjaxServer = function() {
    ...
    var initDatatable = function () {
        var table = $('#ajax-datatable-order');
        const documentProcurementTitle = 'Liste commandes'
        datatable = $(table).DataTable({
            ...
            columns: [
                ...
                {data: 'order_status',
                    render: function ( data, type, row ) {
                        if (data == 0) {
                            return '<span class="ms-2 badge py-3 px-4 fs-7 badge-light-primary fw-bold">Creér</span>';
                        }
                        if (data == 1) {
                            return '<span class="ms-2 badge py-3 px-4 fs-7 badge-light-info fw-bold">Expédié</span>';
                        }
                        if (data == 2) {
                            return '<span class="ms-2 badge py-3 px-4 fs-7 badge-light-info fw-bold">En cours</span>';
                        }
                        if (data == 3) {
...
function functionConfirmOrder(id, url, datatable_id) {
    Swal.fire({
        text: "Voulez-vous envoyer  cette commande?",
        icon: "info",
        ...
        if (result.value) {
            $.ajax({
                ...
                success: function(res){
                    ...
                    oTable.fnDraw(false);
                    Swal.fire({
                        text: "Votre commande a été envoyer avec succès.",
                        icon: "success",
                        buttonsStyling: !1,
...
function functionCloseOrder(id, url, datatable_id) {
    Swal.fire({
        text: "Voulez-vous clôturer la commande?",
        icon: "info",
        showCancelButton: true,
        buttonsStyling: false,
        confirmButtonText: "Confirmer!",
