    ...
    function ProductAuPanier(donnees){
        ...
        $('#tabpanier-id').append(
            '<tr id="tab_ligne-'+p+'">'+
                ...
                '<td class="w-300px">'+
                    '<select name="product_unit[]" class="form-select h-30px p-1 rounded-0 product_unit"  onchange="change_unit_get_price(this);" id="unit_id'+donnees.id+'"></select>' +
                '</td>'+
                '<td class="w-300px">'+
...
const picker = new tempusDominus.TempusDominus(document.getElementById("date_commande_id"), {
    localization: {
       /* locale: "de",*/
        startOfTheWeek: 1,
        format: "yyyy-MM-dd",
