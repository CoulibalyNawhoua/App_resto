var KTpdv = function () {
    ...
    function ProductAuPanier(donnees){
        ...
        $('#tabpanier-id').append(
            ...
            '<td class="w-400px">'+donnees.nom_produit+'</td>'+
            '<td class="w-300px">'+
            ...
            '</td>'+
            '<td class="w-400px">'+
            '<select name="operation[]" style="background-color: #e9ecef"  class="form-select form-select-solid h-30px p-1 rounded-0 motif"  required id="operation_id'+p+'"></select>' +
            '</td>'+
            '<td class="w-300px">'+
        ...
        var operation_id = "operation_id"+p;
        unit_product_select(unit_id, donnees.id);
        selectionnerOperationTypeOutputStock(operation_id);
    };
    ...
    var searchEngine  = new Bloodhound({
        ...
        remote: {
            url: "/search-product-matiere-premiere?q=query",
            wildcard: '%QUERY%',
